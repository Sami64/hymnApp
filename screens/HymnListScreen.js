import { View, Text, FlatList, Dimensions, TextInput, StyleSheet, TouchableOpacity } from 'react-native'
...
export default function HymnListScreen({navigation}) {
      ...
      const item =({item})=>{
        return(
          <TouchableOpacity onPress={()=>navigation.navigate("HymnScreen")}
          style={styles.blockcontainer}>
             <Text style ={styles.blocknumber}>{item.number}</Text>
             <View style={styles.blocktitle}>
                 <Text numberOfLines={1} style={styles.blocktitleEnglish}>{item.EnglishTitle}</Text>
                 <Text numberOfLines={1} style={styles.blocktitleTwi}>{item.TwiTitle}</Text>
             </View>
         </TouchableOpacity>
        );
    };
      ...
      const searchHymn = (input)=>{
       ...
       let searchData = data.filter((item)=>{
         return (item.EnglishTitle.toLowerCase().includes(input.toLowerCase())||item.TwiTitle.toLowerCase().includes(input.toLowerCase()))
       });
       setData(searchData);
      }
    return (
